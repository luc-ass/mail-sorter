...
def move_emails(mailbox, rule, max_emails_per_rule):
    ...
    emails_sorted = sorted(emails, key=lambda msg: msg.date)[:max_emails_per_rule]  # Oldest to newest
    print(f"Found {len(emails_sorted)} matching emails in '{input_folder}', limited to {max_emails_per_rule}")
    ...
    if uids_to_move:
        ...
        mailbox.move(uids_to_move, rule['output_folder'])  # Move all emails in a single batch

    print(f"Completed rule: Moved {len(uids_to_move)} emails from '{input_folder}' to '{rule['output_folder']}'")
    return len(uids_to_move)

def main():
    # Extract global defaults from the configuration
    global_max_emails_per_rule = sorting_config.get("defaults", {}).get("max_emails_per_rule", 10)
        ...
        with MailBox(server).login(username, password) as mailbox:
            ...
            for rule in sorting_config["rules"]:
                if server_name in rule["servers"]:
                    ...
                    max_emails_per_rule = rule.get("max_emails_per_rule", global_max_emails_per_rule)
